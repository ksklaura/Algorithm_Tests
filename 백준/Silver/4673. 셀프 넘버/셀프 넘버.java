...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int i_num;                                                              // 셀프넘버 아닌 숫자 = 원래 숫자 + 각 자리수의 숫자들

        for(int i=0; i<N.length; i++){
            s_num = Integer.toString(i);                                        // 정수형인 숫자를 문자열로 형 변환
            i_num = i;                                                          // 셀프넘버 아닌 숫자 = 원래 숫자
            ...
            if(s_num.length() == 1){                                            // 1,3,5,7,9와 같이 한 자리수인 경우
                ...
                for(int j=0; j<s_num.length(); j++){                            // 문자열의 숫자 길이만큼 반복
                    i_num += Integer.parseInt(s_num.valueOf(s_num.charAt(j)));  // 각 자리의 숫자들을 더함.
                }

                if(i_num <= 10000) N[i_num] = 1;                                // 10000 보다 작거나 같으면 원하는 범위값 내인 것.
                                                                                // 해당 인덱스에 1 할당
            }
        }
